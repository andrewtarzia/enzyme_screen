...
class ESOLCalculator:
    ...
    def calc_esol(self, mol):
        """
        Calculate ESOL based on descriptors in the Delaney paper,
        coefficients refit for the RDKit using the routine refit_esol below
        :param mol: input molecule
        :return: predicted solubility
        """
...
def test_on_dls_100():
    ...
    comparison.  Note that this does not remove the structures in DLS-100 that
    are also in the training set.
    :return: None
    """
    esol_calculator = ESOLCalculator()
    df = pd.read_excel(
        "https://risweb.st-andrews.ac.uk/portal/files/251452157/dls_100.xlsx")
    df.dropna(inplace=True)
    PandasTools.AddMoleculeColumnToFrame(df, 'SMILES',
                                         'Molecule', includeFingerprints=False)
    res = []
...
def add_esol_descriptors_to_dataframe(df, smiles_col="SMILES",
...
def demo(truth_col):
    """
    Read the csv file from the Delaney supporting information, calculate ESOL
    using the data file from the supporting material using the original and
    refit coefficients.  Write a csv file comparing with experiment.
    :return: None
    """
    esol_calculator = ESOLCalculator()
    ...
    for mol, val in df[["Molecule", truth_col]].values:
        res.append([val, esol_calculator.calc_esol(mol),
                    esol_calculator.calc_esol_orig(mol)])
    output_df = pd.DataFrame(res, columns=["Experiment", "ESOL Current",
                                           "ESOL Original"])
